...
object Huffman {
  ...
  def times(chars: List[Char]): List[(Char, Int)] = {
    def occurs(char: Char, accu: List[(Char, Int)]): List[(Char, Int)] = {
      if (accu.isEmpty) List((char, 1))
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    def findCharacter(tree: CodeTree, bits: List[Bit]): (Char, List[Bit]) = {
      tree match {
    ...
    else findCharacter(tree, bits) match {
      case (c: Char, leftOverBits: List[Bit]) => c :: decode(tree, leftOverBits)
    }
  }
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    def findCharacterCode(char: Char, tree: CodeTree): List[Bit] = {
      tree match {
        case Fork(left, r, c, w) if chars(left).contains(char) => 0 :: findCharacterCode(char, left)
        case Fork(l, right, c, w) if chars(right).contains(char) => 1 :: findCharacterCode(char, right)
        case Leaf(c, _) => List[Bit]()
      }
    }
    if (text.isEmpty) List[Bit]()
    else findCharacterCode(text.head, tree) ::: encode(tree)(text.tail)
  }
  ...
  def quickEncode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    val codeLookUpTable = convert(tree)

    if (text.isEmpty) List[Bit]()
    else codeBits(codeLookUpTable)(text.head) ::: quickEncode(tree)(text.tail)
  }
}
